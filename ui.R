...
shinyUI(
        pageWithSidebar(
                ...
                sidebarPanel(
                        numericInput('weight', 'Insert your weight in kilograms', 80) ,
                        numericInput('height', 'Insert your height in metres', 1.90, min = 0.2, max = 3, step = 0.01),
                        submitButton('Submit')
                ), 
                mainPanel(
                        p('The Body mass index (BMI) is a measure of body fat based on height and weight'),
                        p('World Health Organization (WHO) proposes the following classification:'),
                        tags$div(
                                tags$ul(
